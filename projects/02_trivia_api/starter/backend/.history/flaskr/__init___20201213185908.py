...
def create_app(test_config=None):
  ...
  def add_question():
    ...
    new_category_id = body.get('category')

    category = Category.query.filter(Category.id == new_category_id).one_or_none()

    try:
      new_question = Question(question=new_question, answer=new_answer, category=category.type, difficulty=new_difficulty)
      new_question.insert()
    except:
      abort(422)
  ...
  def quizzes():
    ...
    if category_id > 0:
      ...
      question = Question.query.filter(Question.id not in previous_questions_ids).order_by(func.random()).first()
